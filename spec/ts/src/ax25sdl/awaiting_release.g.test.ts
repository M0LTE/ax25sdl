...
import { DataLinkAwaitingRelease } from "./awaiting_release.g.js";

describe("DataLinkAwaitingRelease", () => {
  it("source figure", () => {
    expect(DataLinkAwaitingRelease.source.figure).toBe("figc4.3");
  });

  it("transitions are present", () => {
    expect(DataLinkAwaitingRelease.transitions).toHaveLength(20);
  });

  it("t01_dl_disconnect_request", () => {
    const t = DataLinkAwaitingRelease.transitions.find((x) => x.id === "t01_dl_disconnect_request");
    ...
    expect(t.actions[0].verb).toBe("Expedited DM");
  ...
  it("t02_t1_expiry_no", () => {
    ...
    expect(t.actions[1].verb).toBe("DISC (P = 1)");
  ...
  it("t04_all_other_primitives__from_lower_layer", () => {
    const t = DataLinkAwaitingRelease.transitions.find((x) => x.id === "t04_all_other_primitives__from_lower_layer");
  ...
  it("t06_all_other_primitives__from_upper_layer", () => {
    const t = DataLinkAwaitingRelease.transitions.find((x) => x.id === "t06_all_other_primitives__from_upper_layer");
  ...
  it("t10_sabm_received", () => {
    ...
    expect(t.actions[0].kind).toBe("processing");
    expect(t.actions[1].verb).toBe("Expedited DM");
    expect(t.actions[1].kind).toBe("signal_lower");
  });
  ...
  it("t12_disc_received", () => {
    ...
    expect(t.actions[0].verb).toBe("F := P");
    expect(t.actions[0].kind).toBe("processing");
    expect(t.actions[1].verb).toBe("Expedited UA");
    expect(t.actions[1].kind).toBe("signal_lower");
  });
