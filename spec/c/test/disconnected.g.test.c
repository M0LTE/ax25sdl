...
static int test_source_figure(void) {
    ASSERT_STREQ(data_link_disconnected.source.figure, "figc4.1", "source.figure");
    return 0;
}
...
static int test_t01_dl_disconnect_request(void) {
    ...
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t01_dl_disconnect_request") == 0) {
...
static int test_t02_dl_unit_data_request(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t02_dl_unit_data_request") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t03_dl_connect_request(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t03_dl_connect_request") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t04_all_other_primitives__from_upper_layer(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t04_all_other_primitives__from_upper_layer") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t05_all_other_commands(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t05_all_other_commands") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t06_all_other_primitives__from_lower_layer(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t06_all_other_primitives__from_lower_layer") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t07_control_field_error(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t07_control_field_error") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t08_info_not_permitted_in_frame(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t08_info_not_permitted_in_frame") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t09_u_or_s_frame_length_error(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t09_u_or_s_frame_length_error") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t10_ua_received(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t10_ua_received") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
    ...
    ASSERT_STREQ(t->next, "Disconnected", "next");
    ASSERT(t->actions_len == 1, "actions count");
    ASSERT_STREQ(t->actions[0].verb, "DL-ERROR Indication (C,D)", "actions[0].verb");
    ASSERT(t->actions[0].kind == AX25SDL_KIND_SIGNAL_UPPER, "actions[0].kind");
    return 0;
}
...
static int test_t11_ui_received_no(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t11_ui_received_no") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t11_ui_received_yes(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t11_ui_received_yes") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t12_disc_received(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t12_disc_received") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t13_sabm_received_no(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t13_sabm_received_no") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t13_sabm_received_yes(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t13_sabm_received_yes") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t14_sabme_received_no(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t14_sabme_received_no") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
...
static int test_t14_sabme_received_yes(void) {
    const TransitionSpec* t = NULL;
    for (size_t i = 0; i < data_link_disconnected.transitions_len; i++) {
        if (strcmp(data_link_disconnected.transitions[i].id, "t14_sabme_received_yes") == 0) {
            t = &data_link_disconnected.transitions[i];
            break;
        }
    }
