...
public static class TsEmitter
{
    ...
    /// <summary>
    /// Emit the generated <c>ax25-action-verb.g.ts</c> — the closed string-literal
    /// union of every canonical action verb (SP-010 / packet.net#260). Because the
    /// emitted tables already carry the canonical verb string, the union members
    /// are those same strings; typing <c>ActionStep.verb</c> as this union lets a
    /// dispatcher switch exhaustively (a <c>never</c> default catches a missed verb
    /// at compile time).
    /// </summary>
    public static string EmitActionVerbUnion(IEnumerable<string> canonicalVerbs)
    {
        ...
        sb.Append("export type Ax25ActionVerb =\n");
        sb.Append(string.Join("\n", distinct.Select(v => "  | " + TsStringLiteral(v))));
        sb.Append(";\n");
        return sb.ToString();
    }

    /// <summary>
    /// Emit the generated <c>ax25-guard.g.ts</c> — the closed string-literal
    /// union of every canonical guard atom (SP-010 step for guards). A
    ...
    /// atom as this union lets a guard evaluator bind every atom exhaustively (a
    ...
    public static string EmitGuardUnion(IEnumerable<string> canonicalAtoms)
    {
        ...
        sb.Append("export type Ax25Guard =\n");
        sb.Append(string.Join("\n", distinct.Select(a => "  | " + TsStringLiteral(a))));
        sb.Append(";\n");
        return sb.ToString();
    }

    /// <summary>
    /// Emit the generated <c>ax25-event.g.ts</c> — the closed string-literal
    /// union of every AX.25 SDL event (the <c>spec-sdl/events.yaml</c>
    /// catalogue). Typing <c>TransitionSpec.on</c> as this union lets the
    /// orchestrator's event dispatch switch exhaustively (a <c>never</c> default
    /// catches a missed event at compile time).
    /// </summary>
    public static string EmitEventUnion(IEnumerable<string> events)
    {
        ...
        sb.Append("export type Ax25Event =\n");
        sb.Append(string.Join("\n", distinct.Select(e => "  | " + TsStringLiteral(e))));
        sb.Append(";\n");
        return sb.ToString();
    }
